...
|----------|-------|------|--------|------|---------|
| requirement-class-software-dev | Software-development class law + residual stack (posix-sh, local-only); dest approver/fences **None**; coding-style + sudo-wrap peers | class | Active (2.0.0) | `requirement-class-software-dev.md` | 2026-08-25 |
| requirement-bootstrap-chain | Bootstrap chain A0=cli-template → A1=folder-backup → B=take-ownership (domain replace) | architecture | Active (3.0.0) | `requirement-bootstrap-chain.md` | 2026-08-25 |
...
| requirement-three-layer-privilege-model | You + narrow `action` elev; sudoers emit + install-script; **global-only** grant; **no** `--allow-test-local`; per-user fragments; submit workflow | architecture | Active (2.0.0) | `requirement-three-layer-privilege-model.md` | 2026-08-25 |
| requirement-sudoer-json-file | JSON sudoer file SSOT: `/usr/local/bin/take-ownership` only; `action --path <folder> --ownership *` plus `--json` twin; update **appends** folders | architecture | Active (2.0.0) | `requirement-sudoer-json-file.md` | 2026-08-25 |
| requirement-take-ownership-ops | **Ops SSOT**: recursive chown, no symlink follow; `--path` then `--ownership`; refuse-list; TTY guided input | domain-ops | Active (1.0.0) | `requirement-take-ownership-ops.md` | 2026-08-25 |
...
| requirement-shell-cli-interface | Shell CLI interface (commands, flags, dispatch, modes); `action`; submit `--add`/`--update`; generate; `menu`/`main` routed | shell | Active (2.0.0) | `requirement-shell-cli-interface.md` | 2026-08-25 |
| requirement-shell-cli-zero-arguments | Empty argv Type N help (local-only) | shell | Active | `requirement-shell-cli-zero-arguments.md` | 2026-08-25 |
| requirement-shell-cli-default-interaction | Claimed TTY numbered list on `menu`/`main` (case 3; three work rows; test-purpose grant-emit omitted) | shell | Active (2.0.0) | `requirement-shell-cli-default-interaction.md` | 2026-08-25 |
...
| requirement-domain-take-ownership | Domain **surface** SSOT (four pillars); ops defer to take-ownership-ops; submit public inbound; independent generate; grant-emit **test-purpose** | domain | Active (1.0.0) | `requirement-domain-take-ownership.md` | 2026-08-25 |
